...
CSV_FILENAME_TEMPLATE = 'green_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.csv'
CSV_OUTFILE = f'{AIRFLOW_HOME}/green_taxi_trips/csv/{CSV_FILENAME_TEMPLATE}'

URL_PREFIX = 'https://s3.amazonaws.com/nyc-tlc/trip+data/'
URL = URL_PREFIX + CSV_FILENAME_TEMPLATE

PARQUET_FILENAME_TEMPLATE = CSV_FILENAME_TEMPLATE.replace('csv', 'parquet')
PARQUET_OUTFILE = f'{AIRFLOW_HOME}/green_taxi_trips/parquet/{PARQUET_FILENAME_TEMPLATE}'
...
def convert_to_parquet(csv_file, parquet_file):
    ...
    # Path(f'{AIRFLOW_HOME}/green_taxi_trips/parquet').mkdir(parents=True, exist_ok=True) 
...
with DAG(
    'ingest_green_taxi_trips',
    default_args = default_args,
    description="Insert green taxi trips data to GCS",
    ...
    download_csv_file_task=BashOperator(
        ...
        # bash_command=f'mkdir -p {AIRFLOW_HOME}/green_taxi_trips/csv && curl -sSLf {URL} > {CSV_OUTFILE}'
    ...
    upload_to_gcs_task=PythonOperator(
        ...
        op_kwargs={
            ...
            'blob_name' : f'parquet/green_taxi_trips/{PARQUET_FILENAME_TEMPLATE}'
        }
    )
    ...
    remove_files_from_local_task=BashOperator(
        ...
        bash_command=f'cd {AIRFLOW_HOME}/green_taxi_trips/ &&  rm csv/{CSV_FILENAME_TEMPLATE} && rm parquet/{PARQUET_FILENAME_TEMPLATE}'
    )
