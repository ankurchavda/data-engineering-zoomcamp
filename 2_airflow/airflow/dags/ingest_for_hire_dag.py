...
CSV_FILENAME_TEMPLATE = 'fhv_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.csv'
CSV_OUTFILE = f'{AIRFLOW_HOME}/fhv_tripdata/csv/{CSV_FILENAME_TEMPLATE}'
...
PARQUET_OUTFILE = f'{AIRFLOW_HOME}/fhv_tripdata/parquet/{PARQUET_FILENAME_TEMPLATE}'
...
with DAG(
    ...
    download_csv_file_task=BashOperator(
        ...
        # bash_command=f'mkdir -p {AIRFLOW_HOME}/fhv_tripdata/csv && curl -sSLf {URL} > {CSV_OUTFILE}'
    ...
    upload_to_gcs_task=PythonOperator(
        ...
        op_kwargs={
            ...
            'blob_name' : f'parquet/fhv_tripdata/{PARQUET_FILENAME_TEMPLATE}'
        }
    )

    remove_files_from_local_task=BashOperator(
        task_id='remove_files_from_local',
        bash_command=f'cd {AIRFLOW_HOME}/fhv_tripdata/ &&  rm csv/{CSV_FILENAME_TEMPLATE} parquet/{PARQUET_FILENAME_TEMPLATE}'
    )
